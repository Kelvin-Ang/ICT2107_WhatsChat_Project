...
public class ChatApp extends JFrame {
	...
	public ChatApp() {
		...
		ImageIcon image2;
		try {
			image2 = new ImageIcon(decodeToImage(TestImageBase64));
			ImageLabel = new JLabel(image2);
			ImageLabel.setBounds(492, 42, 77, 62);
		...
		createGroupBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					Group newGroup = new Group(IPincrease(groupList.get(groupList.size() - 1).IPAddress),
							createGroup_txt.getText());
					...
					multicastGroup = InetAddress
							.getByName(groupList.get(getRoomCreated(groupList, createGroup_txt.getText())).IPAddress);
